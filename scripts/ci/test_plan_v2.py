...
class Filters:
    def __init__(self, repository_path, commits, ignore_path, testsuite_root,
                 platforms=[], detailed_test_id=True, quarantine_list=None, tc_roots_th=20):
    ...
    def get_plan(self, options, integration=False, use_testsuite_root=True):
        ...
        with open(fname, newline='') as jsonfile:
            ...
            print(suites)
            unfiltered_suites = list(filter(lambda t: TwisterStatus(t.get('status', None)) is TwisterStatus.NONE, suites))
            logging.info(f"Added {len(unfiltered_suites)} suites to plan.")
            self.all_tests.extend(suites)

        # finally, remove generated test plan
    ...
    def find_boards(self):
        ...
        if len(matched_boards) > 20:
            logging.warning(f"{len(matched_boards)} boards changed, this looks like a global change, "
    ...
    def get_remaining_files(self):
        ...
        logging.debug(f"Remaining files: {remaining}")
        return remaining
    ...
    def find_excludes(self):
        ...
        logging.debug(f"files to be ignored: {found}")
        files_not_resolved = list(filter(lambda x: x not in found, self.modified_files))
        logging.debug(f"not resolved files: {files_not_resolved}")

    def post_filter(self):
        logging.info(f"-------------------  post filters --------------")
        _options = []
        if self.full_twister:
            logging.info(f'Need to run full twister...')
            if self.platforms:
                for platform in self.platforms:
                    _options.extend(["-p", platform])
